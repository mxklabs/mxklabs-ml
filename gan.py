...
class Gan:
  ...
  def train(self, iterations=100, epochs_per_round=1, num_samples=100, 
    ...
    for i in range(iterations):
      ...
      fake_y_0s = np.full(shape=(num_samples, 1), fill_value=0)
      fake_y_1s = np.full(shape=(num_samples, 1), fill_value=1)
      ...
      disc_fake_scores = self._discriminator.evaluate(x=fake_x, y=fake_y_0s, verbose=verbose)[1]
      disc_fake_score = disc_fake_scores.mean()
    
    
      
      gan_scores = self._gan_model.evaluate(x=input_, y=fake_y_1s, verbose=verbose)[1]
      ...
      if not train_generator_only and random.random() > 0.5:
        print(f"---- ROUND {i}: discriminator {disc_real_score:.03f} (real) >>{disc_fake_score:.03f}<< (fake), generator {gan_score:.03f} ----")
        self._discriminator.fit(fake_x, fake_y_0s, epochs=epochs_per_round, verbose=verbose)
        continue

      print(f"---- ROUND {i}: discriminator {disc_real_score:.03f} (real) {disc_fake_score:.03f} (fake), generator >>{gan_score:.03f}<< ----")
      self._gan_model.fit(input_, fake_y_1s, epochs=epochs_per_round, verbose=verbose)